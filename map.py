...
from utils.options import str2bool
...
def get_map_options():
    ...
    # Model results
    parser.add_argument('--results_dir', type=str, default='', help="Relative path containing the results of test.py")
    parser.add_argument('--image_width', type=int, default=224, help="Target Image Width")
    parser.add_argument('--image_height', type=int, default=224, help="Target Image Height")
    ...
    '''
      ...
      (height)            *
                    (Right,Bottom)
    '''
    opts.img_size = (opts.image_width, opts.image_height)
    return opts
...
def main(opts):
    ...
    GT_PATH = os.path.join(os.getcwd(), opts.results_dir, 'ground-truth')
    DR_PATH = os.path.join(os.getcwd(), opts.results_dir, 'detection-results')

    # If there are no images then no animation can be shown
    IMG_PATH = os.path.join(os.getcwd(), opts.results_dir, 'images')
    ...
    results_files_path = os.path.join(os.getcwd(), opts.results_dir, 'results')
    ...
    """
    sum_ap, sum_fscore, sum_prec, sum_rec, sum_lamr, sum_mr, sum_fppi = 0, 0, 0, 0, 0, 0, 0
    ...
    # Print / Write Precision
    mean_prec = sum_prec / n_classes
    text_prec = "precision = {0:.2f}%".format(mean_prec * 100)
    results_file_fscore.write(text_prec + "\n")
    print(text_prec)

    # Print / Write Recall
    mean_rec = sum_rec / n_classes
    text_rec = "recall = {0:.2f}%".format(mean_rec * 100)
    results_file_fscore.write(text_rec + "\n")
    print(text_rec)
